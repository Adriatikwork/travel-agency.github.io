      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {destinations.map((destination) => (
          <Link key={destination.id} href={`/destinations/${destination.slug}`}>
            <Card
              className="group overflow-hidden transition-all duration-300 border-gray-200 cursor-pointer hover:shadow-2xl hover:scale-[1.02] hover:-translate-y-1"
            ...
            >
              <div className="relative h-64 overflow-hidden">
                ...
                <div
                  className={`absolute inset-0 flex flex-col items-center justify-center gap-3 transition-all duration-300 ease-out ${
                    hoveredCard === destination.id
                      ? "opacity-100 translate-y-0"
                      : "opacity-0 translate-y-6 pointer-events-none"
                  }`}
                >
                  <div className="bg-white/95 backdrop-blur-md rounded-xl p-5 mx-6 text-center shadow-2xl transform transition-all duration-300">
                    <Info className="h-6 w-6 text-sky-500 mx-auto mb-2" />
                    <p className="text-sm text-gray-700 font-medium mb-3 line-clamp-3 leading-relaxed">
                      {destination.descriptionShort}
                    </p>
                    <Button
                      size="sm"
                      className="bg-sky-500 hover:bg-sky-600 text-white shadow-lg hover:shadow-xl transition-all"
                    >
                      View Details
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  {destination.featured && <Badge className="bg-sky-500 text-white border-0 shadow-lg">Featured</Badge>}
                  {destination.isNew && <Badge className="bg-green-500 text-white border-0 shadow-lg">New</Badge>}
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  ...
                  <p className="text-sm text-white/90 italic drop-shadow-md">{destination.tagline}</p>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  <div>
                    ...
                    {destination.pricing.note && (
                      <p className="text-xs text-gray-500 mt-1">{destination.pricing.note}</p>
                    )}
                  </div>
                  <div className="text-right">
                    ...
                    {destination.duration.specificDates && (
                      <p className="text-xs text-gray-500 mt-1">{destination.duration.specificDates}</p>
                    )}
                  </div>
                </div>
              </CardContent>
