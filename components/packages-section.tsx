...
import { Check, Package, Facebook, Instagram } from "lucide-react"
...
export function PackagesSection({ packages, currency }: PackagesSectionProps) {
  ...
  const renderCard = (pkg: Package) => {
    const badges = []
    if (pkg.isFeatured) badges.push({ text: ui.featured[language], color: "yellow" as const })
    if (pkg.isOnSale) badges.push({ text: ui.onSale[language], color: "blue" as const })

    return (
      <button
        type="button"
        onClick={() => setSelectedPackage(pkg)}
        className="w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#38b6ff] rounded-2xl"
        aria-label={`View details for ${pkg.title[language]}`}
      >
        <UnifiedCard
              ...
              <div className="space-y-1.5">
                ...
                <div className="space-y-1">
                  ...
                  {pkg.inclusions[language].length > 3 && (
                    <div className="text-[10px] text-[#38b6ff] font-semibold ml-4 hover:underline cursor-pointer">
                      +{pkg.inclusions[language].length - 3} {ui.moreInclusions[language]} — tap to view all
                    </div>
                  )}
                </div>
              </div>
          ...
          ctaButton={
            <div className="w-full bg-[#38b6ff] hover:bg-[#1da6f0] text-white rounded-xl py-2.5 text-xs font-semibold flex items-center justify-center gap-2 transition-colors">
